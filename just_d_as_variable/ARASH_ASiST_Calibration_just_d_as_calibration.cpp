...
#include <string>
#include <vector>
#include </home/mohammad/ARASH_ASiST_Calibration_FactorGraph/just_d_as_variable/ARASH_ASiST_Calibration_just_d_as_calibration.h>
...
int main(int argc, char *argv[])
{
    ...
    double alpha = 49.9740 * M_PI/180; 
    double theta = 42.0017 * M_PI/180;
    double L = 0.347019;
    ...
    for (size_t i = 0; i < data.size()-1; i++)
    {
        ...
        graph.add(std::make_shared<ArashAsistFactor>(Symbol('x', 0), alpha, theta, L, groud_truth, phi, psi, arash_asist_noise_model));
    }

    // add initial values for optimization variables
    initial_estimate.insert(Symbol('x', 0), 1.0); // d
    ...
    std::cout << "\nd is: " << (result_LM.at<double>(Symbol('x', 0)) ) << std::endl;
