...
#include <string>
#include <vector>
#include </home/mohammad/ARASH_ASiST_Calibration_FactorGraph/also_d_as_variable/ARASH_ASiST_Calibration_d_as_calibration.h>
...
int main(int argc, char *argv[])
{
    ...
    for (size_t i = 0; i < data.size()-1; i++)
    {
        ...
        graph.add(std::make_shared<ArashAsistFactor>(Symbol('x', 0), Symbol('x', 1), Symbol('x', 2), Symbol('x', 3), groud_truth, phi, psi, arash_asist_noise_model));
    }
    ...
    initial_estimate.insert(Symbol('x', 3), 1.0); // L
    ...
    std::cout << "\nd is: " << (result_LM.at<double>(Symbol('x', 3)) ) << std::endl;
